...
class App extends Component {
  ...
  _renderLayers() {
    ...
    let storesdata = featurePointStore.filter(i => {
      ...
      if (overview.closestStoresToOrigin 
        && overview.closestStoresToOrigin[storeName] 
        && overview.closestStoresToOrigin[storeName].point.properties.id === i.properties.id) {
          return true
      }
    ...
    const scatterplotLayerProperties = new ScatterplotLayer({
      ...
      data: this.state.properties ? featurePointPropertiesLokalbolig : featurePointPropertiesAll,
      ...scatterplotOptions,
  ...
  handlePropertyClick (e) {
    ...
    const closestStoresToOrigin = getClosestStores(featurePointStore, selectedPoint, maxTravelDistance) 
    const fetchDirectionsToStore = Object.values(closestStoresToOrigin).map(store => {
      return axios.get(api_mapbox_direction(transport ,store.coordinateSet))
        .then(res => {
          store.trip = res.data;
          return store
        })
      ...
      .then(res => {
        return res
          ...
          .map(i => {
            ...
            const data = tripCoordinates.map((coords, index) => coords.concat(300 / tripCoordinates.length * index))
  ...
  _animate() {
    ...
    const loopPercentageCompleted = ((timestampBeginning % this.loopTime) / this.loopTime)
    ...
    return (
      <div>
        <Navigation 
          ...
          onToggleTransport={e => {this.setState({ transport: e }); this.handlePropertyClick(overview.event)}}
          onToggleStore={e => this.setState({ stores: {...stores, [e]: !stores[e]}})}
          onToggleProperties={e => this.setState({ properties: !this.props.properties })}
          onToggleAnimate={() => this.handleToggleAnimate()}
          handleUpdatePathScale={e => this.setState({widthMinPixels: e})}
          handleUpdateTravelDistance={e => this.setState({maxTravelDistance: e})}
          handleUpdateAnimationTime={e =>  { this.loopLength = e }}
          handleChangeDataset={e => this.setState({ properties: !this.state.properties })}
          handleUpdateOpacityBlending={e => this.setState({opacityBlending: e})}
        />
        <DeckGL
        ...
        >
          <ReactMapGL
            ...
            mapStyle={"mapbox://styles/lokalbolig/cjwqvhsjf0bx21cqoecexo0es"}
          />
        </DeckGL>
      </div>
    );
